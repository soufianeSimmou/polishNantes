...
document.addEventListener("DOMContentLoaded", function () {
    ...
    menu[4].addEventListener("click", function () {
        var nouvelleURL = "destroy_session.php";
...
Array.from(buttonrdvvv).forEach(function (element, index) {
    element.addEventListener("click", function () {
        ...
        if (guabarie_cache.value == "BERLINE") {
            prixmodel = 30;
        }  if (guabarie_cache.value == "SUV" || guabarie_cache.value == "GRAND BERLINE") {
            prixmodel = 50;
        }  if (guabarie_cache.value == "GRAND SUV") {
            prixmodel = 80;
        }
        // Alert déplacé après l'assignation de prixmodel
       

        element.style.height = "30vh";
        element.style.backgroundPosition = "center center"; // Valeur valide pour backgroundPosition
        element.style.transition = "all 0.3s ease";
        element.style.backgroundColor = "white";

        const childElement = element.querySelector(':first-child');
        if (childElement) {
            childElement.style.backgroundPosition = "center center"; // Valeur valide pour backgroundPosition
            childElement.style.transition = "all 0.3s ease";
...
function annulerStyless(element) {
    if (element) {
        ...
        const childElement = element.querySelector(':first-child');
        if (childElement) {
            childElement.style.backgroundPosition = "";
